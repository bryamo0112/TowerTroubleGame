...
public class AN_DoorScript : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Player"))
        {
            ...
            Player2Controller player2Controller = other.GetComponent<Player2Controller>();
            Player3Controller player3Controller = other.GetComponent<Player3Controller>();

            if ((playerController != null && playerController.hasTool) || 
                (player2Controller != null && player2Controller.hasTool) || 
                (player3Controller != null && player3Controller.hasTool)) // Access hasTool as a property
            {
                Debug.Log("Player with tool detected. Opening the door.");
                Action(); // Open the door when a player with the tool triggers it
            }
            else
            {
                Debug.Log("Player detected but does not have the tool. Door remains closed.");
            }
        }
    }
